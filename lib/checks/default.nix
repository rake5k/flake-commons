...

{
  ...
  shellcheck = pkgs.runCommand "check-shellcheck" { buildInputs = [ pkgs.shellcheck ]; } ''
    mkdir $out
    cd ${flake}
    IFS=$'\n'
  ...
  markdownlint = pkgs.runCommand "check-markdownlint" { buildInputs = [ pkgs.markdownlint-cli ]; } ''
    mkdir $out
    cd ${flake}
    markdownlint .
  '';

  yamllint = pkgs.runCommand "check-yamllint" { buildInputs = [ pkgs.yamllint ]; } ''
    mkdir $out
    cd ${flake}
    yamllint .
  '';
